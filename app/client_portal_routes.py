...
def create_review(
    ...
    """Allow clients to submit a review (requires completed project)"""
    
    # Check if user has at least one completed project
    completed_project = db.exec(
        select(Project)
        .where(Project.owner_id == current_user.id)
    ...
    if not completed_project:
        raise HTTPException(
            status_code=403,
            detail="You must complete a project before leaving a review"
        )
...
def list_approved_reviews(db: Session = Depends(get_session)):
    ...
    out: List[ReviewOut] = []
    for r in reviews:
        user = db.get(PortalUser, r.user_id)
        out.append(
            ReviewOut(
                id=r.id,
                user_id=r.user_id,
...
def admin_list_reviews(
    ...
    """Admin: List all reviews (approved and pending)"""
    reviews = db.exec(
        select(Review).order_by(Review.created_at.desc())
    ).all()
...
class ReviewApproval(SQLModel):
    is_approved: bool


@router.patch("/admin/reviews/{review_id}/approve", response_model=ReviewOut)
def admin_approve_review(
    review_id: int,
